...
public class MySQLMemberDao implements MemberDao{
	...
	public Member findBy(int no) {
		try(PreparedStatement stmt = con.prepareStatement("select member_no, name, email, gender, created_date"
				+ " from myapp_member"
				+ " where member_no=?")) {
	...
	public int update(Member member) {
		try(PreparedStatement stmt = con.prepareStatement("update myapp_member set"
				+ " name=?, "
				+ " email=?,"
				+ " password=sha1(?),"
				+ " gender=?"
	...
	public int delete(int no) {
		try(PreparedStatement stmt = con.prepareStatement("delete from myapp_member where member_no=?")) {
