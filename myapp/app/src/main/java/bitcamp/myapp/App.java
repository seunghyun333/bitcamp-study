...
private <T extends CsvObject> void loadCsv(String filename, List<T> list, Class<T> clazz) {
    try {
      ...
      while ((line = in.readLine()) != null) {
        list.add((T)factoryMethod.invoke(null,line)); // Reflection API를 사용하여 메서드 스테틱 호출 
        //list.add(Member.fromCsv(line)); 직접 스테틱 메서드 호출 
      }
  ...
  private void saveCsv(String filename, List<? extends CsvObject> list) {
    //List<> CsvObject 를 상속받거나 인터페이스로 갖고있는 클래스가 들어올수잇음 
      ...
      for (CsvObject obj : list) {
        ...
        //Board 클래스에 필드가 추가/변경/삭제되더라도 여기 코드를 변경할 필요없다.
        // Information Expert 설계를 적용하는 이유 ,, !! 
