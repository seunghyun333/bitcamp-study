...
// 풀링pooling 기법을 이용하여 DB 커넥션 재활용
// - GoF의 Flyweight패턴 (버리지 않고 재활용)
...
public class DataSource {
  ...
  public Connection getConnection() throws Exception {
	  ...
	  // 현재 스레드의 Connection 보관소에서 Connection 객체를 꺼낸다. 
	  ...
	  if(con==null) {
	   ...
	   // 커넥션 풀에 사용할 Connection 객체가 없다면 새로 생성한다. 
		...
		System.out.printf("[%s]- 새 DB커넥션 생성\n", Thread.currentThread().getName());
		}
  ...
  public void clean() {
	//스레드가 작업을 끝냈으면, 이 스레드에 보관된 Connection 객체를 제거한다.
	 ...
	 if (con != null) {
		 ...
		 // 다음에 다시 사용해야하기 때문에 close()하면 안된다.
