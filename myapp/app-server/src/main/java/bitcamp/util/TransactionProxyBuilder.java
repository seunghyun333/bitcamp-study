...
public class TransactionProxyBuilder {
    ...
    return Proxy.newProxyInstance(
            ...
            new InvocationHandler() {
              ...
              public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                // 프록시 객체의 메서드를 호출할 때,
                // @Transactional 이 붙은 메서드를 호출할 때는 TransactionTemplate으로 처리하고
                // @Transactional 이 안 붙은 메서드는 그냥 오리지널 객체의 메서드를 그대로 호출한다.
                // System.out.printf("%s - 트랜젝션 경유!\n", originalMethod.getName());
                ...
                if (transactional != null) {
                  //@Transactional 애노테이션이 붙은 메서드라면 TransactionTemplate를 통해 실행하고
                  ...
                  //@Transactional 이 안 붙은 메서드라면 그냥 호출한다.
  ...
  public Method getOriginalMethod(Object originalWorker, Method method) throws Exception{
    ...
    for (Method originalMethod : methods){
      if(originalMethod.getName().equals(method.getName()) && method.getParameterCount() == method.getParameterCount()) {
        return method;
      }
    }
    return null;
