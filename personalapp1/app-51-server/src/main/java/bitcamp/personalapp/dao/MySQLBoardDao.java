...
public class MySQLBoardDao implements BoardDao {
  ...
  public void insert(Board board) {
    try (PreparedStatement stmt =
    		ds.getConnection(false).prepareStatement("insert into personalapp_board(title,content,writer,password)"
            + " values(?,?,?,sha1(?))")) {
  ...
  public List<Board> list() {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement("select" + " b.Board_no," + " b.title, "
  ...
  public Board findBy(int no) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement("select" + " b.Board_no," + " b.title, b.content,"
        + " b.view_count, " + " b.created_date, " + " v.visit_No, " + " v.name" + " from"
        + " personalapp_board b inner join personalapp_visit v on b.writer=v.visit_no"
        + " where board_no=?"
  ...
  public int update(Board board) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement("update personalapp_board set" + " title= ?,"
